...
public class TTSNew : MonoBehaviour
{
    [SerializeField]
    ...
    private void Start()
    {
        ...
        if (externalInputField != null)
        {
            ...
            {
                externalInputField.onEndEdit.AddListener(delegate { AddTextToQueue(externalInputField.textComponent); });
            }
            else
            {
                externalInputField.onValueChanged.AddListener(delegate { AddTextToQueue(externalInputField.textComponent); });
            }
        }

        inputField = gameObject.AddComponent<InputField>();
        inputField.textComponent = gameObject.AddComponent<Text>();
        inputField.onValueChanged.AddListener(delegate { AddTextToQueue(inputField.textComponent); });
    }
    private void Update()
    {
    ...
    public void AddTextToQueue(Text text)
    {
        Debug.Log("AddTextToQueue: " + text.text);
        if (!string.IsNullOrEmpty(text.text))
        {
            textQueue.Enqueue(text.text);
